...
export class QrCodeComponent {
  ...
  async generateQRCode() {
    ...
    const employeeData = {
      ...
      jobTitle: `${this.userData.roles}`,
      email: this.userData.email,
      phone: this.userData.contactNo,
      employeeId: this.userData.empNo
    };

    // Create attendance URL
    const dataParam = encodeURIComponent(JSON.stringify(employeeData));
    this.attendanceUrl = `${environment.baseUrl}/employeeAttendence/mark-attendance/present/${this.userData.empNo}`;
